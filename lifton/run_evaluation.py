...
def initialize_lifton_gene_eval(
    ...
    if not args.evaluation_liftoff_chm13:
        ...
        lifton_gene = lifton_class.Lifton_GENE(
            ref_gene_id,
            copy.deepcopy(locus),
            copy.deepcopy(ref_db["gene-" + ref_gene_id].attributes),
            tree_dict,
            ref_features_dict,
            args,
            tmp=with_exons,
...
def lifton_add_trans_exon_cds_eval(
    ...
    if not args.evaluation_liftoff_chm13:
        ...
        lifton_trans = lifton_gene.add_transcript(
            ref_trans_id,
            copy.deepcopy(locus),
            copy.deepcopy(ref_db["rna-" + ref_trans_id].attributes),
        )
...
def evaluation(
    ...
    """
    exon_children = list(
        tgt_db.children(locus, featuretype="exon", level=1, order_by="start")
    )
    if lifton_gene is None and ENTRY_FEATURE:
        # Gene (1st) features
        lifton_gene, ref_gene_id, ref_trans_id = initialize_lifton_gene_eval(
            locus,
            ref_db,
            tree_dict,
            ref_features_dict,
    ...
    if len(exon_children) == 0:
        ...
        lifton_trans, cds_num = lifton_add_trans_exon_cds_eval(
            lifton_gene, locus, ref_db, tgt_db, ref_trans_id, args
        )
        if lifton_trans == None:
            return lifton_gene
        lifton_trans_aln, lifton_aa_aln = lifton_gene.orf_search_protein(
            ...
            lifton_status,
            eval_only=True,
            eval_liftoff_chm13=args.evaluation_liftoff_chm13,
        )
        lifton_utils.print_lifton_status(
            lifton_trans.entry.id, locus, lifton_status, DEBUG=args.debug
        )
