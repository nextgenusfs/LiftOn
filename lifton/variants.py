...
def find_variants(align_dna, align_protein, lifton_status, peps, is_non_coding):
    ...
    if is_non_coding:
        ...
        return
    if align_dna is None:
        mutation_type.append("full_transcript_loss")
        lifton_status.status = mutation_type
        return
    if align_protein is None:
    ...
    if is_frameshift(align_dna.query_aln):
        ...
        frameshift = True
    if is_frameshift(align_dna.ref_aln) and frameshift is False:
        mutation_type.append("frameshift")
        frameshift = True
    if (
        align_dna.query_aln[0:3] != align_dna.ref_aln[0:3]
        and align_dna.query_aln[0:3] != "ATG"
        and align_protein.query_aln[0] != align_protein.ref_aln[0]
        and align_protein.query_aln[0] != "M"
    ):
