...
def exec_miniprot(outdir, args, tgt_genome, ref_proteins_file):
    ...
    if miniprot_annotation is None or not os.path.exists(miniprot_annotation):
        ...
        print("**********************")
        miniprot_annotation = run_miniprot.run_miniprot(
...
def LiftOn_eval_alignment(
    eval_trans, locus, tgt_fai, ref_proteins, ref_trans_id, lifton_status
):
...
def LiftOn_liftoff_alignment(
    lifton_trans, locus, tgt_fai, ref_proteins, ref_trans_id, lifton_status
):
...
def LiftOn_miniprot_alignment(
    ...
    if (ref_trans_id in m_id_dict.keys()) and (ref_trans_id in ref_proteins.keys()):
        ...
        for m_id in m_ids:
            ...
            _, overlap = segments_overlap_length(
                (m_entry.start, m_entry.end), (transcript.start, transcript.end)
            )
            ...
            ##################################################
            ovps_liftoff = tree_dict[chromosome].overlap(
            ...
            tmp_m_lifton_aln = align.lifton_parasail_align(
        ...
        for locus in ref_db.db_connection.features_of_type(f_itr):
            ...
            if len(exon_children) > 0:
                ...
                for transcript in list(transcripts):
                    ...
                    all_CDS_in_trans = list(
                        ref_db.db_connection.children(
                            transcript, featuretype="CDS", order_by="start"
                        )
                    )
            ...
            ref_features_dict[
                locus.id if not args.evaluation_liftoff_chm13 else locus.id[5:]
            ] = feature
            all_CDS_children = list(
                ref_db.db_connection.children(
            ...
            if len(all_CDS_children) > 0:
                ref_features_len_dict[
                    locus.id if not args.evaluation_liftoff_chm13 else locus.id[5:]
                ] = all_CDS_children[-1].end - all_CDS_children[0].start + 1
            else:
                ref_features_len_dict[
                    locus.id if not args.evaluation_liftoff_chm13 else locus.id[5:]
                ] = 0
...
def get_ref_ids_miniprot(
    ref_features_reverse_dict, miniprot_trans_id, m_id_2_ref_id_trans_dict
):
...
def write_lifton_eval_status(fw_score, transcript_id, transcript, lifton_status):
    ...
    fw_score.write(
        f"{transcript_id}\t{lifton_status.eval_dna}\t{lifton_status.eval_aa}\t{lifton_status.annotation}\t{final_status}\t{transcript.seqid}:{transcript.start}-{transcript.end}\n"
    )
...
def print_lifton_status(transcript_id, transcript, lifton_status, DEBUG=False):
    final_status = ";".join(lifton_status.status)
    logger.log(
        f"{transcript_id}\t{lifton_status.liftoff}\t{lifton_status.miniprot}\t{lifton_status.lifton_dna}\t{lifton_status.lifton_aa}\t{lifton_status.annotation}\t{final_status}\t{transcript.seqid}:{transcript.start}-{transcript.end}\n",
        debug=DEBUG,
    )
...
def write_lifton_eval_status(fw_score, transcript_id, transcript, lifton_status):
    final_status = ";".join(lifton_status.status)
    fw_score.write(
        f"{transcript_id}\t{lifton_status.lifton_dna}\t{lifton_status.lifton_aa}\t{final_status}\t{transcript.seqid}:{transcript.start}-{transcript.end}\n"
    )
...
def check_ovps_ratio(mtrans, mtrans_interval, overlap_ratio, tree_dict):
    ...
    for ovp in ovps:
        ovp_len, _ = segments_overlap_length(
            (mtrans_interval[0], mtrans_interval[1]), (ovp[0], ovp[1])
        )
