...
def args_gffutils(parser):
    ...
    gffutils_grp.add_argument(
        ...
        choices=["create_unique", "merge", "error", "warning", "replace"],
...
def args_aligngrp(parser):
    ...
    aligngrp.add_argument(
        ...
        metavar="F",
        type=float,
        help="amount of flanking sequence to align as a "
...
def args_optional(parser):
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="THREADS",
        help="use t parallel processes to accelerate alignment; by default p=1",
    )
    parser.add_argument("-m", help="Minimap2 path", metavar="PATH")
    parser.add_argument(
        "-f", "--features", metavar="TYPES", help="list of feature types to lift over"
    )
    parser.add_argument(
        "-infer-genes",
        required=False,
    ...
    parser.add_argument(
        "-chroms",
        metavar="TXT",
        help="comma seperated file with corresponding chromosomes in "
    ...
    parser.add_argument(
        ...
        default=0.1,
        metavar="O",
        help="maximum fraction [0.0-1.0] of overlap allowed by 2 "
    ...
    parser.add_argument(
        ...
        default=1,
        metavar="GE",
        help="gap extend penalty in exons when finding best "
        "mapping; by default GE=1",
        type=int,
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="annotate status of each CDS "
...
def parse_args(arglist):
    ...
    parser.add_argument(
        "-E",
        "--evaluation",
        help="Run LiftOn in evaluation mode",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "-EL",
        "--evaluation-liftoff-chm13",
        help="Run LiftOn in evaluation mode",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "-c",
        "--write_chains",
        help="Write chaining files",
        action="store_true",
        default=True,
    )
    parser.add_argument(
        "--no-orf-search",
        help="Do not perform open reading frame search",
        action="store_true",
        default=False,
    )
    ...
    ###################################
    referenceseqgrp = parser.add_argument_group(
    ...
    miniprotrefrgrp = parser.add_argument_group(
    ...
    parser.add_argument(
        ...
        "--annotation-database",
        metavar="SOURCE",
        help="The source of the reference annotation (RefSeq / GENCODE / others).",
        default="RefSeq",
    )
    ###################################
...
def run_all_lifton_steps(args):
    ...
    ) = lifton_utils.get_ref_liffover_features(features, ref_db, intermediate_dir, args)
    ...
    if (
        (ref_proteins_file is None)
        or (not os.path.exists(ref_proteins_file))
        or (ref_trans_file is None)
        or (not os.path.exists(ref_trans_file))
    ):
        logger.log(
        ...
        ref_trans, ref_proteins = extract_sequence.extract_features(
            ref_db, features, ref_fai
        )
        ref_proteins_file = lifton_utils.write_seq_2_file(
            intermediate_dir, ref_proteins, "proteins"
        )
        ref_trans_file = lifton_utils.write_seq_2_file(
            intermediate_dir, ref_trans, "transcripts"
        )
    else:
        logger.log(
            ">> Reading transcript DNA dictionary from the reference fasta ...",
            debug=True,
        )
        logger.log(
            ">> Reading transcript protein dictionary from the reference fasta ...",
            debug=True,
        )
    ...
    logger.log(
        "\t\t * number of truncated proteins: ",
        len(trunc_ref_proteins.keys()),
        debug=True,
    )
    ...
    if args.evaluation:
        ...
        tgt_feature_db = annotation.Annotation(
            tgt_annotation, args.infer_genes
        ).db_connection
        fw_score = open(lifton_outdir + "/eval.txt", "w")
        tree_dict = {}
        processed_features = 0
        for feature in features:
            for locus in tgt_feature_db.features_of_type(
                feature
            ):  # , limit=("chr1", 146652669, 146708545)):
                lifton_gene = run_evaluation.evaluation(
                    None,
                    locus,
                ...
                if processed_features % 20 == 0:
                    sys.stdout.write(
                        "\r>> LiftOn evaluated: %i features." % processed_features
                    )
    ...
    miniprot_annotation = lifton_utils.exec_miniprot(
        lifton_outdir, args, tgt_genome, ref_proteins_file
    )
    ...
    logger.log(
        f">> Creating miniprot annotation database : {miniprot_annotation}", debug=True
    )
    ...
    ref_id_2_m_id_trans_dict, m_id_2_ref_id_trans_dict = (
        lifton_utils.miniprot_id_mapping(m_feature_db)
    )
    ...
    for feature in features:  # CP132235.1:34100723-34103135
        for locus in l_feature_db.features_of_type(
            feature
        ):  # , limit=("chr1", 206759465, 206890507)):
            lifton_gene = run_liftoff.process_liftoff(
                None,
                locus,
                ref_db.db_connection,
                l_feature_db,
    ...
    if args.measure_time:
        ...
        print(
            f"Creating reference annotation database: {creating_reference_annotation_database}"
        )
        print(
            f"Get all reference features to liftover: {get_all_reference_features_to_lift}"
        )
        ...
        print(
            f"Process Liftoff genes & transcripts: {process_liftoff_genes_transcripts}"
        )
        ...
        fw_time.write(
            f"{reading_target_reference_genomes}\tReading target & reference genomes\n"
        )
        fw_time.write(
            f"{creating_reference_annotation_database}\tCreating reference annotation database\n"
        )
        fw_time.write(
            f"{get_all_reference_features_to_lift}\tGet all reference features to liftover\n"
        )
        fw_time.write(
            f"{extract_protein_dna_dictionaries}\tExtract protein & DNA dictionaries\n"
        )
        ...
        fw_time.write(
            f"{miniprot_2_liftoff_id_mapping}\tMiniprot 2 Liftoff ID mapping\n"
        )
        fw_time.write(
            f"{process_liftoff_genes_transcripts}\tProcess Liftoff genes & transcripts\n"
        )
