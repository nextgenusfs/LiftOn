...
def lifton_miniprot_with_ref_protein(
    ...
    Lifton_trans = lifton_gene.add_miniprot_transcript(
        ref_trans_id,
        copy.deepcopy(m_feature),
        ref_db[ref_trans_id].attributes,
        ref_features_dict,
    )
    lifton_gene.update_trans_info(
        Lifton_trans.entry.id, m_feature.seqid, m_feature.start, m_feature.end
    )
    # Create exon / CDS entries
    cdss = m_feature_db.children(
        m_feature, featuretype="CDS"
    )  # Replace 'exon' with the desired child feature type
    ...
    m_lifton_aln = align.lifton_parasail_align(
        Lifton_trans, m_entry, tgt_fai, ref_proteins, ref_trans_id
    )
...
def process_miniprot(
    ...
    is_overlapped = lifton_utils.check_ovps_ratio(
        mtrans, mtrans_interval, args.overlap, tree_dict
    )
    lifton_gene = None
    lifton_trans = None
    if not is_overlapped:
        ...
        if ref_trans_id in ref_proteins.keys() and ref_trans_id in ref_trans.keys():
            if ref_trans_id is not None:
                ...
                if (
                    len(list(m_feature_db.children(mtrans, featuretype="CDS"))) == 1
                    and ref_trans_exon_num_dict[ref_trans_id] > 1
                ):  # Processed pseudogene
                    return None
                miniprot_trans_ratio = (
                    mtrans.end - mtrans.start + 1
                ) / ref_features_len_dict[ref_gene_id]
                if (
                    miniprot_trans_ratio > args.min_miniprot
        ...
        lifton_utils.print_lifton_status(
            transcript_id, mtrans, lifton_status, DEBUG=args.debug
        )
