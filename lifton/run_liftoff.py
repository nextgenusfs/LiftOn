...
def initialize_lifton_gene(
    locus, ref_db, tree_dict, ref_features_dict, args, with_exons=False
):
    """
    ...
    lifton_gene = lifton_class.Lifton_GENE(
        ref_gene_id,
        copy.deepcopy(locus),
        copy.deepcopy(ref_db[ref_gene_id].attributes),
        tree_dict,
        ref_features_dict,
        args,
        tmp=with_exons,
...
def lifton_add_trans_exon_cds(lifton_gene, locus, ref_db, l_feature_db, ref_trans_id):
    ...
    lifton_trans = lifton_gene.add_transcript(
        ref_trans_id,
        copy.deepcopy(locus),
        copy.deepcopy(ref_db[ref_trans_id].attributes),
    )
...
def process_liftoff_with_protein(
    locus,
    lifton_gene,
    lifton_trans,
    ...
    """
    # Liftoff alignment
    liftoff_aln = lifton_utils.LiftOn_liftoff_alignment(
        lifton_trans, locus, tgt_fai, ref_proteins, ref_trans_id, lifton_status
    )
    # miniprot alignment
    miniprot_aln, has_valid_miniprot = lifton_utils.LiftOn_miniprot_alignment(
        locus.seqid,
        locus,
        ref_id_2_m_id_trans_dict,
        m_feature_db,
    ...
    if liftoff_aln is None:
        ...
        if has_valid_miniprot:
            ...
            if write_chains:
                lifton_utils.write_lifton_chains(
                    fw_chain, lifton_trans.entry.id, chains
                )
...
def process_liftoff(
    ...
    """
    exon_children = list(
        l_feature_db.children(locus, featuretype="exon", level=1, order_by="start")
    )
    ...
    if len(exon_children) == 0:
        ...
        if ENTRY_FEATURE:  # Gene (1st) features with direct exons
            ...
            ref_gene_id, ref_trans_id = lifton_utils.get_ref_ids_liftoff(
                ref_features_dict, lifton_gene.entry.id, locus.id
            )
        ...
        try:  # Test if the reference transcript exists. Skip if not.
            ...
            return None
        lifton_trans, cds_num = lifton_add_trans_exon_cds(
            lifton_gene, locus, ref_db, l_feature_db, ref_trans_id
        )
        if cds_num > 0:
            process_liftoff_with_protein(
                locus,
                lifton_gene,
                lifton_trans,
                ref_id_2_m_id_trans_dict,
        ...
        lifton_trans_aln, lifton_aa_aln = lifton_gene.orf_search_protein(
