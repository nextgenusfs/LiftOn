...
def process_m_l_children(
    ...
    m_matches, m_length = get_id_fraction.get_partial_id_fraction(
        m_lifton_aln.ref_aln,
        m_lifton_aln.query_aln,
        math.floor(m_aa_start),
        math.ceil(m_aa_end),
    )
    l_matches, l_length = get_id_fraction.get_partial_id_fraction(
        l_lifton_aln.ref_aln,
        l_lifton_aln.query_aln,
        math.floor(l_aa_start),
        math.ceil(l_aa_end),
    )
...
def create_lifton_entries(
    ...
    for c_idx in (
        range(m_c_idx_last, m_c_idx)
        if miniprot_is_better
        else range(l_c_idx_last, l_c_idx)
    ):
